...
public class MatchParser
{
    ...
    private Result FunctionVariable(String s) throws Exception
    {
        String f = "";
        int i = 0;
        // search for function name or variable name
        // name has to start from the letter
        while (i < s.length() && (Character.isLetter(s.charAt(i)) || ( Character.isDigit(s.charAt(i)) && i > 0 ) )) {
            f += s.charAt(i);
            i++;
        }
        if (!f.isEmpty()) { // if we found smth.
            if ( s.length() > i && s.charAt( i ) == '(') { // and next symbol is bracket - we found function
                Result r = Bracket(s.substring(f.length()));
                return processFunction(f, r);
            } else { // otherwise - it's variable
    ...
    private Result Num(String s) throws Exception
    {
        ...
        // only numbers & dots are allowed
        while (i < s.length() && (Character.isDigit(s.charAt(i)) || s.charAt(i) == '.')) {
            // only 1 dot in number is allowed!
            if (s.charAt(i) == '.' && ++dot_cnt > 1) {
                throw new Exception("not valid number '" + s.substring(0, i + 1) + "'");
            }
            i++;
        }
        if( i == 0 ){ // we didn't found any valid number
    ...
    private Result processFunction(String func, Result r)
    {
        ...
        else if (func.equals("tgh")){
            return new Result(Math.tanh(r.acc), r.rest);
        }
        else if (func.equals("ctgh")){
